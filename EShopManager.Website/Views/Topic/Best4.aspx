...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <%
        ...
        var BestB = Model.Listing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.B).ToList();
        var BestE = Model.Listing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.E).ToList();
        var BestS = Model.Listing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.S).ToList();
        var BestT = Model.Listing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.T).ToList();

        var GroupB = Model.GroupListing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.B).ToList();
        var GroupE = Model.GroupListing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.E).ToList();
        var GroupS = Model.GroupListing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.S).ToList();
        var GroupT = Model.GroupListing.Where(x => x.Type == EShopManager.Core.Constancy.BestType.T).ToList();
    %>
    <table class="body0 StepBody">
        <tr>
            <td>
                ...
                <!-- Template end-->
                <div id="body2-a">
                    2. BEST 分類結果 <span id="notice" style="background-color: rgb(255, 195, 195);" class="hidden">
                        資料已更新</span>
                </div>
                <div id="body3">
